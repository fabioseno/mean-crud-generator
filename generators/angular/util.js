...
function getListViewHTMLGridColumns(config) {
    ...
    listingFieldsOutput = format.formatText('[{0}];', columns.join(', ')) + os.EOL;
...
function getDetailsViewHTMLFields(config) {
    ...
    for (var i = 0; i < config.fields.length; i++) {
        ...
        switch (field.controlType) {
            ...
            case 'tel':
            case 'currency':
                controls += format.setTabs(7) + format.formatText('<mat-label>{0}</mat-label>', field.fieldLabel) + os.EOL;
                controls += format.setTabs(7) + format.formatText('<input matInput {2}name="{0}" {1}{5}[(ngModel)]="{3}.{0}"{4}>', field.fieldName, required, tel, config.entityName, currency, maxlength) + os.EOL;
                break;
            case 'checkbox':
                controls += format.setTabs(7) + format.formatText('<mat-checkbox name="{0}" color="primary" [(ngModel)]="{1}.{0}">{2}</mat-checkbox>', field.fieldName, config.entityName, field.fieldLabel) + os.EOL;
                break;
            case 'calendar':
                break;
            case 'textarea':
                controls += format.setTabs(7) + format.formatText('<mat-label>{0}</mat-label>', field.fieldLabel) + os.EOL;
                controls += format.setTabs(7) + format.formatText('<textarea matInput name="{0}" rows="4" {2}{3}[(ngModel)]="{1}.{0}"></textarea>', field.fieldName, config.entityName, required, maxlength) + os.EOL;
                break;
        }
